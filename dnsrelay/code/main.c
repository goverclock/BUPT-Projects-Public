...
int main(int argc, char *argv[]) {
    ...
    while (1) {
        ...
        if (msg_len <= 0) continue;  // no message received
        ...
        debug_time(1);
        debug(1, " %c type: ", header->qr ? 'A' : 'Q');
        if (question.qtype == TYPE_A)
            debug(1, "A\t");
        else
            debug(1, "%d\t", question.qtype);
        debug(1, " from: ");
        ...
        if (header->qr == 0) {  // question from local progress
            ...
            if (found) {
                ...
                debug(1, "ID: %d -> %d\t", origin_id, header->id);
            }
            ...
            if (!found) {
                ...
                debug(1, "found in cache\t\t");
                debug_qname(1, question.qname);
                debug(1, " -> ? <- ");
                debug_ip(1, rr.rdata);
            } else if (found & (1 << 1)) {
                debug(1, "found in table\t\t");
                debug_qname(1, question.qname);
                debug(1, " -> ? <- ");
                debug_ip(1, rr.rdata);
            }
            ...
            // 2. A type
            if (header->opcode != 0 || ntohs(header->qdcount) != 1) {
                debug(1, "\n");
                continue;
            }
            ...
            debug(1, "ID: %d <- %d\t", header->id, serv_id);
            debug_qname(1, question.qname);
            debug(1, " <- ");
...
void argument_resolv(int argc, char *argv[]) {
    ...
    while ((opt = getopt(argc, argv, "d::a:f:")) != -1) {
        if (opt == 'd') {
            ...
            printf(
                "usage: [-d | -dd] [-a <dns-server-ipaddr>] [-f <filename>]\n");
